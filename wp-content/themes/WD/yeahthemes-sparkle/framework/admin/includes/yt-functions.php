...
if( !function_exists( 'yt_beautify_number' ) ) {
	function yt_beautify_number($n, $precision = 3) {
	    if ($n < 1000000) {
 ...
 * @since     1.0
 */
function yt_pretty_print( $array = array() ) {
...
if ( ! function_exists( 'yt_get_embed' ) ) {
	function yt_get_embed( $url, $default = '' ){
		...
		return !empty( $embed_html ) ? $embed_html : __( '<span>Embed HTML not available.</span>', 'yeahthemes' );
...
if ( ! function_exists( 'yt_wp_ajax_get_embed' ) ) {
	function yt_wp_ajax_get_embed() {
		...
		wp_send_json_success( yt_get_embed( esc_url( $url ) ) );
...
if( !function_exists( 'yt_parse_google_fonts' ) ) {
	function yt_parse_google_fonts( $json = '' ){
		$json = $json ? $json : get_template_directory_uri() . '/framework/extended/webfonts.json';
		...
		$fonts_raw;
		$fonts_data = array();
		$transient_name = 'yt_transient_parsed_webfonts_' . yt_clean_string( basename( yt_clean_url( $json ) ), '-', '_' );
		...
		if ( empty(  $transient_value ) ) {
			...
			if( is_wp_error( $response ) ) {
				...
				$fonts_raw = wp_remote_retrieve_body( $response );
			}
			...
			if( $fonts_raw && yt_is_json( $fonts_raw ) ){
				...
				set_transient( $transient_name, $fonts_data, 24 * HOUR_IN_SECONDS );
			}
		}else{
			$fonts_data = $transient_value;
			//print_r($fonts_data);
			//echo 'cached';
		}
	...
	function yt_used_typography( $data = array(), $type = 'googlefont' ){
		...
		foreach( $data as $k => $v ){
			...
			if( !empty( $v['face'] ) && strpos( $v['face'], $type . '-' ) !== false && $type === $type ){
				...
				if( isset( $v['style'] ) && in_array( $v['style'] , array( 'oblique', 'italic' ) ) ){
					$variants[] = 'italic';
				}
...
if( !function_exists( 'yt_output_option' ) ) {
	function yt_output_option( $type, $data ){
		...
		if( 'background_options' === $type ){
			...
			if( !empty( $data['image'] ) ){
				$output .= "\tbackground-image:url(". yt_clean_url( $data['image'] ).");\n";
		...
		elseif( 'background_image' === $type ){
			$output .= !empty( $data )
				? "\tbackground-image:url(". yt_clean_url( $data ).");\n" 
				: '';
		}
		/*Typography */
