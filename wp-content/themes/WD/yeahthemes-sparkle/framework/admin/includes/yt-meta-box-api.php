	...
	function show_metabox() {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			if( !in_array( $field['type'], array( 'tab', 'tab_break' ) ) ){
				...
				echo '<div class="', $section_class, '"' . $attr_datafold . '>';
					
				if( $this->_meta_box['show_names'] == true ) {
					
					echo '<div class="yt-field-label"><label for="', $field['id'], '">', $field['name'], '</label></div>';
					echo '<div class="yt-controls ">';
				
				}else{
					echo '<div class="yt-field-label yt-hidden"><label for="', $field['id'], '">', $field['name'], '</label></div>';
					echo '<div class="yt-controls ">';
			...
			$_args = array(
				...
				case 'tab':
					...
					if( $_tab_count == 1 ){
						...
						echo '<!--yt-group-tab-wrapper--><div class="yt-group-tab-wrapper" id="yt-group-tab-' . yt_clean_string( $this->_meta_box['id'], '_', '-' ) . yt_clean_string( join( '-', $_tab_array[$_tab_break_loop_counter] ) , '-', '' ) . '"'. ( isset( $field['settings']['cookie']) && 0 == $field['settings']['cookie'] ? ' data-cookie="0"' : '') .'>';
							...
							if( !empty( $_tab_array[$_tab_break_loop_counter] ) ){
								...
								foreach( $_tab_array[$_tab_break_loop_counter] as $tab_k => $tab_v ){
									...
									echo '<li data-index="' . esc_attr( $tab_k + 1 ) . '"><span>' . $tab_v . '</span></li>';
