	...
	*/
	function __construct() {
		...
		$widget_ops = array(
			...
			'description' => __('Display widgets inside a tab', 'yeahthemes')
		);
		...
		parent::__construct( 
			...
			__('(Theme) Smart Tabby widget', 'yeahthemes'), 
			$widget_ops, 
			$control_ops
		);
	...
	function form( $instance ) {
		...
		$default = array( 
			'title' => __('Smart widget','yeahthemes'),
			'widgets' =>  '',
			'size'	=> 'small',
		);
		...
		<em><?php _e('Title only show for "small" size.', 'yeahthemes') ?></em>
		<p>
			<label for="<?php echo $this->get_field_id( 'size' ); ?>"><?php _e('Title Size', 'yeahthemes') ?></label>
			<select id="<?php echo $this->get_field_id( 'size' ); ?>" name="<?php echo $this->get_field_name( 'size' ); ?>">
				<option value="small" <?php if ( 'small' == $instance['size'] ) echo 'selected="selected"'; ?>><?php _e('Small','yeahthemes')?></option>
				<option value="big" <?php if ( 'big' == $instance['size'] ) echo 'selected="selected"'; ?>><?php _e('Big','yeahthemes')?></option>
			</select>
		</p>
		<em><?php _e('Size option is for tab only.', 'yeahthemes') ?></em>
	...
	function yt_display_widgets( $args, $instance ){
		...
		if( !empty($widgets) && is_array($widgets) ){ ?>
		<?php if ( $title && 'small' == $instance['size'] ){ echo $before_title . $title . $after_title; } ?>
		<div class="yt-tabby-tabs">

			<div class="yt-tabby-tabs-header slashes-navigation<?php echo 'big' == $instance['size'] ? esc_attr( ' widget-title' ) : '';?>">
				...
				foreach ($widgets as $widget) {
					...
					if( !empty( $widget ) ) {
						...
						if( isset( $nested_widget ['id_base'] ) && $widget ) {
							...
							<div class="<?php echo esc_attr( 'widget_'.$nested_widget ['widget-id'].' '.$widget_options['classname'] ); ?> <?php echo $active ?>">
							<?php  
								$default_args = array( 
