...
add_action( 'template_redirect', 'yt_ajax_responser'  );
/**
 ...
 * @since 1.0
 */
if ( ! function_exists( 'yt_ajax_responser' ) ) {
	function yt_ajax_responser() {
...
if ( ! function_exists( 'yt_section_classes' ) ) {
	function yt_section_classes( $class = '', $pos = '' ) {
		...
		if(yt_get_section_classes($class, $pos)){
			...
			$output .= 'class="' . join( ' ', yt_get_section_classes( $class, $pos ) ) . '"';
		...
		/* Extra attribute */
		$yt_extra_attribute = '';
		$yt_extra_attribute = yt_apply_atomic( $pos . '_attribute', $yt_extra_attribute);
		
		if($yt_extra_attribute){
			$output .= ' ' . $yt_extra_attribute;
		}
...
if ( ! function_exists( 'yt_get_section_classes' ) ) {
	function yt_get_section_classes($class = '', $pos = '') {
		...
		return yt_apply_atomic( $pos . '_class', $classes, $class );
	}
}
...
if( !function_exists( 'yt_default_body_class' ) ) {
	...
	function yt_default_body_class($classes) {
		...
		if( !class_exists( 'Mobile_Detect' ) ){
			...
			load_template( YEAHTHEMES_FRAMEWORK_DIR . 'extended/class.mobile-detect.php' );
...
if( !function_exists( 'yt_admin_body_class' ) ) {
	...
	function yt_admin_body_class( $classes ) {
		...
		/* Allow reducing admin motion */		
		$yt_admin_motion = apply_filters( 'yt_admin_motion', (bool) true );
		
		if( $yt_admin_motion ){
			
			$admin_body_class[] = 'yt-admin-motion-enabled';
