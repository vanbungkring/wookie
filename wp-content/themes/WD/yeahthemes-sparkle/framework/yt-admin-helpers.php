...
if( !function_exists( 'yt_get_revslider' ) ) {
	...
	function yt_get_revslider(){
		//global $wpdb;
		//$revTableName = $wpdb->prefix . "revslider_sliders";
		//$response = $wpdb->get_results( "SELECT title,alias FROM $revTableName" );
		//return $response;
...
if( !function_exists( 'yt_get_icon_font_content' ) ) {
	function yt_get_icon_font_content( $url ){
		...
		if( $icon_font_url === '' ) {
			...
			$result[''] =  __( 'No icon font found!', 'yeahthemes' );
			return $result;
			 ...
			 */
				...
				if( is_wp_error( $remote_content ) ) {
					...
					$result[''] = __( 'No icon found!', 'yeahthemes' );
				}
				else{ 
				
					$result[''] = __( 'Select icon:', 'yeahthemes' );
...
if( !function_exists( 'yt_twitter_user_profile' ) ) {
	function yt_twitter_user_profile( $user = 'Yeahthemes' ){
		$transName = 'yt_transient_twitter_profile_'. $user;
		$cacheTime = 30; 
...
if( !function_exists( 'yt_twitter_user_timelines' ) ) {
	function yt_twitter_user_timelines($count = 3, $user = 'Yeahthemes'){
		...
		$transName = 'yt_transient_twitter_tweets_'. $user . '_count_' . $count;
		$cacheTime = 10; 
		//echo $transName;
		...
		if ( empty(  $transient_value ) ) {
			...
			foreach( $tweets as $tweet ){
				...
				$tweet_date = yt_twitter_time( $tweet['created_at'] );
 ...
 * @since     1.0
 */
if( !function_exists( 'yt_twitter_user_timeline' ) ) {
	function yt_twitter_time( $time ) {
		...
		if(is_numeric($d) && $d > 0) { 
			...
			if( $d < 3 ) return __( 'right now', 'yeahthemes' ); 
			//if less then minute 
			if( $d < $minute ) return floor( $d ) . __( ' seconds ago', 'yeahthemes' ); 
			//if less then 2 minutes 
			if( $d < $minute * 2 ) return __( 'about a minute ago', 'yeahthemes' ); 
			//if less then hour 
			if( $d < $hour ) return floor( $d / $minute ) . __(' minutes ago', 'yeahthemes' );
			//if less then 2 hours 
			if( $d < $hour * 2 ) return __('about an hour ago', 'yeahthemes' ); 
			//if less then day 
			if( $d < $day ) return floor( $d / $hour ) . __( ' hours ago','yeahthemes' ); 
			//if more then day, but less then 2 days 
			if( $d > $day && $d < $day * 2 ) return __( 'yesterday','yeahthemes' ); 
			//if less then year 
			if( $d < $day * 365 ) return floor( $d / $day ) . __( ' days ago', 'yeahthemes' ); 
...
if( !function_exists( 'yt_get_social_network_counts' ) ) {
	function yt_get_social_network_counts( $service = '', $id = '' ){
		...
		$transient_name = sprintf( 'yt_transient_social_network_counts_%s_%s', $service , $id );
		...
		if( !empty( $transient )){
			...
			switch ( $service) {
				...
				case 'facebook':
					$response_data = wp_remote_get( "https://graph.facebook.com/$id", array( 'sslverify'=>false ) );
				...
				case 'youtube':
					$response_data = wp_remote_get( "https://gdata.youtube.com/feeds/api/users/$id?alt=json", array( 'sslverify' => false ) );
					$response_data = !is_wp_error( $response_data ) ? wp_remote_retrieve_body( $response_data ) : '';
					$response_data_content = $response_data ? json_decode( $response_data, true ) : '';
				...
				case 'googleplus':
					$response_data = wp_remote_get("https://www.googleapis.com/plus/v1/people/$id?key=$google_api", array( 'sslverify' => false ));  
					$response_data = !is_wp_error( $response_data ) ? wp_remote_retrieve_body( $response_data ) : ''; 
					$response_data_content = $response_data ? json_decode( $response_data, true ) : '';
