...
if( !function_exists( 'yt_theme_font_size_changer') ) {
	function yt_theme_font_size_changer( $extra_class = ''){
		...
		?>
		<div class="yt-font-size-changer<?php echo $extra_class;?>">
...
if ( ! function_exists( 'yt_direction_nav' ) ) {
	...
	function yt_direction_nav( $nav_id ) {
		...
		?>
		<nav role="navigation" id="<?php echo esc_attr( $nav_id ); ?>" class="<?php echo $nav_class; ?> direction-nav hidden-print primary-2-secondary">
			<h4 class="screen-reader-text hidden"><?php _e( 'Post navigation', 'yeahthemes' ); ?></h4>
	
		<?php if ( is_single() ) : // navigation links for single posts ?>
	
			<?php previous_post_link( '<div class="nav-previous">%link</div>', '<span class="meta-nav">' . $icon_prev . '</span> <span class="pager-title">%title</span>' ); ?>
			<?php next_post_link( '<div class="nav-next">%link</div>', '<span class="pager-title">%title</span> <span class="meta-nav">' . $icon_next . '</span>' ); ?>
	
		<?php elseif ( $wp_query->max_num_pages > 1 && ( is_home() || is_archive() || is_search() ) ) : // navigation links for home, archive, and search pages ?>
	
			<?php if ( get_next_posts_link() ) : ?>
			<div class="nav-previous"><?php next_posts_link( '<span class="meta-nav">' . $icon_prev . '</span> <span class="pager-title">' . __('Older Entries', 'yeahthemes') . '</span>'); ?></div>
			<?php endif; ?>
	
			<?php if ( get_previous_posts_link() ) : ?>
			<div class="nav-next"><?php previous_posts_link( '<span class="pager-title">' . __('Newer Entries', 'yeahthemes') . '</span> <span class="meta-nav">' . $icon_next . '</span>' ); ?></div>
			<?php endif; ?>
...
if ( ! function_exists( 'yt_pagination_nav' ) ) {
	function yt_pagination_nav() {
	?>
	<nav role="navigation" class="secondary-2-primary pagination-nav hidden-print clearfix">
		<h4 class="screen-reader-text hidden"><?php _e( 'Post navigation', 'yeahthemes' ); ?></h4>
		<div class="pull-left">
			<?php
				global $wp_query;
		...
		<div class="pull-right hidden-xs">
			...
			<?php
				...
				echo sprintf( __( 'Page %d of %d','yeahthemes' ), $paged, $wp_query->max_num_pages );
			...
			<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
				<footer class="comment-meta">
					<div class="comment-author vcard">
						...
						<?php printf( __( '%s', 'yeahthemes' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>
					</div><!-- .comment-author -->
...
if ( ! function_exists( 'yt_post_meta_description' ) ) {
	...
	function yt_post_meta_description() {
		
		$default_meta = yt_apply_atomic( 'theme_default_post_meta_description', true );
		if( !$default_meta ){
			
			yt_do_atomic('yt_theme_post_meta_description');
			...
			printf( 
				__( '<span class="posted-on">%1$s</span><span class="by-author"> by %2$s</span><span class="in-cat"> in %3$s</span>', 'yeahthemes' ),
...
if( !function_exists( 'yt_get_the_post_format_video') ) {
	function yt_get_the_post_format_video( $post_id = null ){
		...
		if( $output ){
			...
			$poster_url = '' == $poster_url ? apply_filters( 'yt_mejs_default_media_cover', YEAHTHEMES_CINCLUDES_URI . 'images/media-player-cover.jpg' ) : $poster_url;
